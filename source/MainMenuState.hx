...
class MainMenuState extends MusicBeatState
{
	...
	{			
		...
		if (!selectedSomethin)
		{
			...
			if (FlxG.keys.justPressed.B #if mobile || controls.LEFT_P #end && bopCount == 0) {
				var b = new FlxSprite(150).loadGraphic(Paths.image("B"));
				add(b);
				bopCount++;
			}
				
			if (FlxG.keys.justPressed.O #if mobile || controls.RIGHT_P #end && bopCount == 1) {
				var o = new FlxSprite(300).loadGraphic(Paths.image("O"));
				add(o);
				bopCount++;
			}

			if (FlxG.keys.justPressed.P #if mobile || vPad.buttonC.justPressed #end && bopCount == 2) {
