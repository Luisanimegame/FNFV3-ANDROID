...
class HitBox extends FlxSpriteGroup
{
  ...
  function createHitbox(x:Float, y:Float, width:Int, height:Int, color:String)
  {
	...
	if (SaveManagement.getOption("Hitbox Invisible") == "Nope"){
    button.alpha = 0.1;
    ...
    if (SaveManagement.getOption("Hitbox Invisible") == "Yes"){
    button.alpha = 0.0001;

    button.onDown.callback = () -> button.alpha = 0.0001;

    button.onUp.callback = () -> button.alpha = 0.0001;
    }
